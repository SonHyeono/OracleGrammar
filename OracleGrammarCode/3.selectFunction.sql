...
/*
    내장 함수 종류
        1. 입력한 행수(row) 수 만큼 결과 반환
        2. 집계(다중행, 그룹) 함수 - 입력한 행수와 달리 결과 값 하나만 반환
*/
...
select round(5.66, 1), round(5.66, -1), 5.6 from dual;

-- 3. 지정한 자리수 이하 버리는 함수 : trunc()  (내림)
-- trunc(데이터, 자릿수)
-- 자릿수 : +(소수점 이하), -(정수 의미)
...
select ename from emp where job = upper('manager');  --이게 맞음. 왜냐면 job을 바꾸면 전체 데이터를 바꿔야하는데 
select ename from emp where lower(job) = 'manager';     -- 생각을 하자. 조건에서 데이터 셋을 바꾸는게 아니라 데이터를 데이터셋에 맞추는 것이 중요
...
select '  ab  ', length('  ab  '), length(trim('  ab  ')) from dual;
...
-- [3] to_number(문자열, 변환포맷) : 문자열을 숫자로 변환
...
-- select to_number('20000') - to_number('10000') from dual; 오류
...
select ename, sal, decode(job, 'ANALUST', sal*1.05,
                          'SALESMAN', sal*1.1,
                          'MANAGER', sal*1.15,
                          'CLERK', sal*1.2,
                          sal) as 인상분
