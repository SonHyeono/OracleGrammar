...
-- emp & dept 
-- 존재하는 각 table의 모든 칼럼 만큼 기준점 없이 검색
-- 즉 조인시에는 기준이 되는 조건 비교가 필요
-- 논리적인 오류 
...
-- step 01 : 상사가 없는 KING의 이름 검색이 안되었음!!!
select e.ename,  m.ename 상사명
from emp e, emp m
where e.mgr  = m.empno;

-- step 02 : 상사가 없는 KING의 이름 검색이 되도록.
/* KING은 상사가 없음 - null
...
-- 03.
/* sal이 3000이상인 데이터들을 두개의 table에서 검색시에 deptno는 두개의 테이블 모두에 존재
 *	+ 표기 의미가 없다
*/
SELECT d.deptno AS 부서번호, dname, loc, empno, ename, job, mgr, hiredate, sal, comm, e.deptno AS 소속부서번호
FROM emp e, dept d 
 ...
 * 
 * e.deptno(+) = d.deptno
 * 	- dept의 deptno가 30번/40번 검색
 * 
 * e.deptno = d.deptno(+)
 *  - dept의 deptno가 30번/40번 미검색
 * 
 * sal(+) : null을 보유한 sal도 검색되게 하는 문법
 * 	dept table의 detptno는 null이 존재하지 않음
 * 	emp의 deptno는 sal(+) >= 3000 조건에 해당하는 10과 20번 부서 이외에는 null로 간주
 * 30번과 40번 검색시에 어차피 emp의 검색된 deptno는 null 결론 : emp의 deptno가 null 즉 데이터가 없다
 *  + 기호는 데이터 없는 쪽에 적용
 */
SELECT d.deptno AS 부서번호, dname, loc, empno, ename, job, mgr, hiredate, sal, comm, e.deptno AS 소속부서번호
FROM emp e, dept d WHERE e.deptno(+)=d.deptno AND sal(+) >= 3000;

-- 05. dept table의 deptno가 30번, 40번이 누락되어 검색
SELECT d.deptno AS 부서번호, dname, loc, empno, ename, job, mgr, hiredate, sal, comm, e.deptno AS 소속부서번호
FROM emp e, dept d WHERE e.deptno = d.deptno(+) AND sal(+) >= 3000;


-- 미션? 사원이 없는 부서의 정보를 검색
-- 사번이 null인 사원의 부서번호가 미존재(null)하는 deptno 값을 보유한 table은?
/*
deptno 	  dname   loc
---------------------
...
직원 table에는 직원들 존재
단, 직원이 부서에 미포함(부서 id가 null) 되었다 하더라도 모든 검색시에는
departments에는 null이라는 department_id 값이 없기 때문에 null이라는 id없는 table 컬럼에 + 기호 표기

*/

select first_name, hiredate, department_name
...
select   e.ename as 사원명 , e.empno, e.mgr, m.ename as 상사명
from emp e, emp m
where e.mgr = m.empno(+)
