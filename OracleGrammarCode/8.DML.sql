...
- insert/update/delete 문장의 필수 항목
	...
	- 트랜젝션
		...
		- application으로 개발시에 db에 insert 후에 commit 안하고 데이터 검색시 검색 불가로 문제 발생
...
--? 11, 유재석, 10 emp01에 / 22, 백종원, 20 emp02
...
-- 서브쿼리 사용
-- loc에 무의미한 데이터로 갱신 시도시에 0 row update 메세지 받음
-- 유의미한 데이터 갱신시 3 row update 메세지 받음
drop table emp01;
create table emp01 as select * from emp;
