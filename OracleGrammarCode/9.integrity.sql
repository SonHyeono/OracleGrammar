...
-- 모델링 하는 table들의 제약조건 설정은 개발자의 책임
...
4. 제약조건 선언 위치
	...
	4-3. 이미 생성된 table에 제약 조건 추가 및 수정
		...
		- 현업에선 model tool로 모델링 필수로 작업 
			...
			- 대부분의 tool table 생성 후 alter 명령어로 제약조건
			추가 형식으로 sql 생성
...
-- 1.  딕셔너리 table 검색
-- 해당 db의 어떤 제약조건이 있는지 확인 되는 table
...
-- 참고 : 현업에선 not null은 제약조건명 설정 하지 않음, 단순 문법 습득을 위한 설정으로 제시
drop table emp02;
...
-- empno는 pk(제약 조건 이름 명시) / ename는 not null(제약조건명 불명시)
-- empno : emp02_empno_pk, 타입 p
-- ename : SYS_C007015(oracle마다 숫자가 다를수있음) 오라클이 자체적으로 이름 부여, 타입 C
drop table emp02;
create table emp02(
	empno number(4),
	ename varchar2(10) not null,
...
insert into emp02 values(1, '재석', 10);
-- insert into emp02 values(2, '재석', 60); 에러 부모엔 60 데이터가 없어서 에러가 남.



--7. 6번의 내용의 모든 제약조건을 table 레벨 단위로 설정해 보기
drop table emp02;
drop table emp02;
create table emp02(
	empno number(4),
...
-- dept와 emp와 같이 dept01과 emp01을 제약조건 추가
...
-- 자식 table에서 사용되는 데이터를 부모 table 삭제 시도시 에러 발생
...
--11.참조되는 자식이 부모의 특정 컬럼을 사용(참조)한다 하더라도 데이터라 하더라도 삭제 가능한 명령어
...
-- 13.? gender라는 컬럼에는 데이터가 M 또는(or) F(in)만 저장되어야 함
drop table emp01;
