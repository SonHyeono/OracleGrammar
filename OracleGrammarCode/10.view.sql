...
2. 개념
	...
	예시
		- emp의 comm은 반드시 받는 자만 인지, 존재자체도 모르는 직원들도 있다 가정시
			- 일반 직원들에게 comm 은닉 필수
...
5. 종류
	...
	5-3. 인라인 view : sql의 from 절에 view 문장 (select 검색된 결과 - DB관점에선 논리적인 table)
...
-- join 된 sql문장을 위한 view 는 사용빈도가 높음
-- insert/update/delete는 잘 안 하고 select 위주로 검색시에 권장
/* join 로직 필요시 개발 방법
방법 1 - 필요시 마다 원본 table로 직접 select
방법 2 - 이미 조인해서 생성한 view에 select

실행 속도 면에서 view만 select 하는 게 더 효율적
*/
/* 개발 방법
- 두개의 join 필수
...
--6. 논리적인 가상의 table이 어떤 구조로 되어 있는지 확인 가능한 oracle  자체 table
	-- view는 text 기반으로 명령어가 저장 
		-- table 생성 명령어는 별도로 저장해주지 않음, view 명령어는 내부적으로 text로 저장해 놓음
